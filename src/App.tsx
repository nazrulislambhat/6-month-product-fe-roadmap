...
export default function Roadmap() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const [progressResult, themeResult] = await Promise.all([
          window.storage.get(STORAGE_KEY),
          window.storage.get(THEME_KEY),
        ]);
        if (progressResult?.value) setChecked(JSON.parse(progressResult.value));
        if (themeResult?.value) setDark(themeResult.value === "dark");
      } catch (_) {
  ...
  // ── Save progress whenever checked changes (after initial load) ────────────
  useEffect(() => {
    if (!loaded) return;
    const save = async () => {
  ...
  useEffect(() => {
    if (!loaded) return;
    window.storage.set(THEME_KEY, dark ? "dark" : "light").catch(() => {});
  }, [dark, loaded]);
  ...
  if (!loaded) {
    return (
      <div style={{ minHeight: "100vh", background: "#07070e", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ color: "#444", fontFamily: "monospace", fontSize: 12, display: "flex", gap: 10, alignItems: "center" }}>
          <span style={{ animation: "spin 1s linear infinite", display: "inline-block" }}>⟳</span> Loading your progress…
